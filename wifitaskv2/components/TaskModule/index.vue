  ...
  <div class="module">
    ...
    <div class="main">
      ...
      <div v-else class="container">
        <!-- <ActivityRule v-if="showActivityRule" @close="showActivityRule = false" showMode="bottomReturn"/> -->
        <TaskBundle v-if="taskModuleTab === 'TaskBundle'" :bundles :showkey @choose-bundle="chooseBundle"
          @refresh="updatePageData('TaskBundle', false)" />
        <TaskDetail v-if="taskModuleTab === 'TaskDetail'" :tasks :bundle :showkey
          @refresh="updatePageData('TaskDetail', false)" />
        <NearWiFi v-if="taskModuleTab === 'NearWiFi'" v-model="wifiList" :address
          @refresh="updatePageData('NearWiFi')" />
...
import { clusterListData, taskListData, taskRecordData, scanWifiListData } from '../../mockdata'
...
// const showActivityRule = ref(!hasShowActivityRule)

const getNearbyPackageInfoFun = async () => {
  // bundles.value = clusterListData
  // showLoading.value = false
  // setTimeout(() => { showkey.value++ }, 1000)
  // emit('init-complete')
...
const getTaskInfoInPackageFun = () => {
  // tasks.value = taskListData
  // showLoading.value = false
  // setTimeout(() => { showkey.value++ }, 1000)
  ...
  getTaskInfoInPackage({
    ...
    log('getTaskInfoInPackage', res.data, { diffTime: new Date().getTime() - now })
...
const getUserTasksFun = () => {
  // userTasks.value = taskRecordData
  // showLoading.value = false
...
const getNearbyWifiTaskFun = () => {
  realtimeReport.reportRealTime('TaskModuleInit_GetNearbyWifiTaskFun')
  log('getNearbyWifiTaskFun-hash:', window.location.hash)
  // wifiList.value = scanWifiListData
  // showLoading.value = false
  getLocation().then((location) => {
    realtimeReport.reportRealTime('TaskModuleInit_GetLocation', {}, {
      location: location
    })
    log('getLocation-location', location)
    const { latitude: lat, longitude: lng } = location
    ...
    getScanWifiList().then((res) => {
      realtimeReport.reportRealTime('TaskModuleInit_GetScanWifiList', {}, {
        scanWifiList: res
      })
      log('getScanWifiList-res', res)
      const { wifiList: scanWifiList } = res
      ...
      // const scanWifiName = ["文叔叔小龙虾"]
      log('scanWifiName', scanWifiName)
      return scanWifiName
    }).then((scanWifiName) => {
      if(scanWifiName.length > 0) {
        return getNearbyWifiTask({ scanWifiName })
      } else {
        return showNearbyWifiTask()
      }
    }).then((apiRes) => {
      ...
      wifiList.value = []
      showLoading.value = false
      realtimeReport.reportRealTime('TaskModuleInit_GetNearbyWifiTask_Error', {}, {
        err
      })
    })
  })
