...
const clickRightButton = async () => {
  ...
  try {
    ...
    if (result.code === 0) {
      ...
      realtimeReport.reportRealTime('connectwifi_need_delete', { connectWifiResult: result })
      setTimeout(() => {
        nextTick(() => { showDeleteWifi.value = true })
      }, 100)
    } else {
      realtimeReport.reportRealTime('connectWifi_error', { connectWifiResult: result })
...
const retryReportWifi = async (params, retries = 1) => {
  try {
    ...
    if (error.message && error.message.include('TypeError: Failed to fetch')) {
      realtimeReport.reportRealTime('reportwifi_network_err')
    } else {
      realtimeReport.reportRealTime('reportwifi_other_err', { reportError: errorToJson(error) })
    }
...
const handleSuccess = async (noVerify = false) => {
  realtimeReport.reportRealTime('connectwifi_success', { ssid: props.authWifi.wifiName, noVerify })
